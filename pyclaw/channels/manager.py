...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        if self.config.channels.openim.enabled:
            try:
                from pyclaw.channels.openim import OpenIMChannel
                self.channels["openim"] = OpenIMChannel(
                    self.config.channels.openim, self.bus
                )
                logger.info("OpenIM channel enabled")
            except ImportError as e:
                logger.warning(f"OpenIM channel not available: {e}")
